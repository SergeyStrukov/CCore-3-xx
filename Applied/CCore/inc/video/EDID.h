...
inline constexpr ulen EDIDLen = 128 ;
...
struct EDIDTimingDesc
 {
  ...
  EDIDTimingDesc() noexcept {}

  explicit EDIDTimingDesc(const uint8 buf[18]);

  // properties

  bool isInterlaced() const { return flags&0x80u; }
  ...
  SyncType syncType() const { return SyncType( (flags>>3)&0x02u ); }

  bool hsyncPositive() const { return flags&0x02u; } // Digital...

  bool vsyncPositive() const { return flags&0x04u; } // DigitalSeparate

  unsigned freq() const { return (pixel_clock*1000u)/((hlen+hblank)*(vlen+vblank)); }
...
struct EDIDBlockDesc
 {
  ...
  EDIDBlockDesc() noexcept : index(Desc_none) {}
