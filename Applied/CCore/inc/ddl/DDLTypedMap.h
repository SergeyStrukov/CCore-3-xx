...
struct MapTypeListCheck<T,TT...>
 {
  ...
  static bool Match(const TypeSet &type_set,TypeList *type_list)
   {
    if( type_list->count != 1 + sizeof ... (TT) ) return false;
...
class TypedMap : NoCopy
 {
  ...
  private:
   ...
   using ConstAlgo = typename RBTreeLink<RecConst,NameKey>::template Algo<&RecConst::link> ;
