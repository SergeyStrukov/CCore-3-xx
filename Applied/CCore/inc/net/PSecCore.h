...
class KeySet : NoCopy
 {
   ...
   SimpleArray<Rec> key_set;
   SimpleArray<uint8> key_buf;
   SimpleArray<KeyIndex> active_list;
  ...
  private:
   ...
   static KeyIndex NextSerial(KeyIndex serial) { return KeyIndex( (serial+1)&3 ); }
...
class ProcessorCore : NoCopy
 {
  ...
  private:
   ...
   void feed(const uint8 block[ /* BLen */ ]) { random_engine.feed(Range(block,blen)); }
