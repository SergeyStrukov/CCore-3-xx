...
using ICMPEchoNumber = uint16 ;

using ICMPReplyFunction = Function<void (ICMPEchoNumber num)> ;

using IPTickFunction = Function<void (void)> ;

using UDPFunction = Function<void (Packet<uint8> packet,PtrLen<const uint8> data,UDPoint udpoint)> ;
...
class ARPTable : NoCopy
 {
   ...
   using TreeAlgo = TreeLink<Entry,uint32>::RadixAlgo<&Entry::tlink> ;
   using ListAlgo = DLink<Entry>::CircularAlgo<&Entry::dlink> ;
...
class ARPending : NoCopy
 {
  ...
  public:
   ...
   using StatInfo = CancelPacketList::StatInfo ;
...
class ICMPEchoList : NoCopy
 {
   ...
   using Algo = TreeLink<ICMPEchoEndpoint,uint16>::RadixAlgo<&ICMPEchoEndpoint::link> ;
...
class IPTickList : NoCopy
 {
   ...
   using Algo = DLink<IPTickEndpoint>::CircularAlgo<&IPTickEndpoint::link> ;
...
class UDPList : NoCopy
 {
   ...
   using Algo = TreeLink<UDPEndpoint,UDPort>::RadixAlgo<&UDPEndpoint::link> ;
