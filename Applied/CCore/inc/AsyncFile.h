...
class AsyncFileSystem : public Funchor_nocopy
 {
  ...
  private:
   ...
   void complete_time(PacketHeader *packet);

   PacketFunction function_complete_time() { return FunctionOf(this,&AsyncFileSystem::complete_time); }

   Packet<uint8,Sys::AsyncFileSystem::CmpFileTimeExt> getPacket_time(const char *name,TimeScope time_scope);
