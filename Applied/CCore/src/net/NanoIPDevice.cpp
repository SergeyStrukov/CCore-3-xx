...
uint16 ICMPEchoEndpoint::send()
 {
  ...
  echo.setCheckSum(Range(data));

  dev->send_icmp_echo(dst,echo,Range(data));
...
const Unid NanoIPDevice::TypeUnid={{0xB299'8408,0x75B7'E4E4,0x05BF'4E33,0x25FB'6384,0x0FE4'3A5C,0xC4F5'B768,0xA602'657E,0x781D'5A62}};
...
IPEvent NanoIPDevice::inbound_udp(Packet<uint8,EthRxExt> packet,const IPHeader &iph,PtrLen<uint8> body)
 {
  ...
  return hook.inbound(packet.forgetExt<1>(),body,iph,udph);
 }
...
NanoUDPEndpointDevice::Engine::Engine(UDPoint dst_)
 : dst(dst_),
   host("NanoUDPEndpointDevice","NanoUDPEndpointDevice")
 {
 }
...
void NanoUDPEndpointDevice::Engine::attach(InboundProc *proc)
 {
  host.attach(proc);
 }
...
PacketFormat NanoUDPEndpointDevice::getOutboundFormat() const
 {
...
ulen NanoUDPEndpointDevice::getMaxInboundLen() const
 {
...
void NanoUDPMultipointDevice::Engine::inbound(Packet<uint8> packet,PtrLen<const uint8> data,UDPoint udpoint)
 {
  ...
  if( +hook )
    {
     hook->inbound(udpoint.get(),packet,data);
    }
  else
    {
     packet.complete();
...
NanoUDPMultipointDevice::Engine::Engine()
 : host("NanoUDPMultipointDevice","NanoUDPMultipointDevice")
 {
 }
...
void NanoUDPMultipointDevice::Engine::attach(InboundProc *proc)
 {
  host.attach(proc);
 }
...
StrLen NanoUDPMultipointDevice::toText(XPoint point,PtrLen<char> buf) const
 {
  PrintBuf out(buf);
...
PacketFormat NanoUDPMultipointDevice::getOutboundFormat() const
 {
  return udp_endpoint.getUDPTxFormat();
 }
...
ulen NanoUDPMultipointDevice::getMaxInboundLen() const
 {
  return udp_endpoint.getMaxInboundLen();
 }
