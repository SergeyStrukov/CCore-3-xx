...
const Unid ClientDevice::TypeUnid={{0x5888'C3C7,0x93FD'EE19,0x3791'C740,0x6116'85BA,0x780E'24F9,0x355E'9A91,0xE36D'0FC9,0x8FE7'779C}};
...
ClientDevice::ClientDevice(StrLen ptp_dev_name)
   ...
   ptp(hook)
 {
  ...
  getFileUpdateTime_format=ptp->getFormat_guarded<GetFileUpdateTimeExt>(name);
...
void ClientDevice::getFileUpdateTime(Packet<uint8,GetFileUpdateTimeExt> packet,StrLen path)
 {
  if( packet.checkDataLen(getFileUpdateTime_format,path.len) )
    {
     packet.setDataLen(getFileUpdateTime_format,path.len).copyFrom(path.ptr);

     GetFileUpdateTimeInput input(path.len);
