...
void ClientEngine::connection_lost()
 {
  abortAll();

  Hook hook(host);

  if( +hook ) hook->connection_lost();
 }

void ClientEngine::connection_close()
 {
  abortAll();

  Hook hook(host);

  if( +hook ) hook->connection_close();
 }
...
ClientEngine::ClientEngine(ulen max_slots)
 : mutex("PTP::ClientEngine"),
   ...
   host("PTP::ClientEngine","PTP::ClientEngine.host")
 {
  ep=0;
...
void ClientEngine::attach_to(PacketEndpointDevice *ep_)
 {
  ...
  const char *name="CCore::Net::PTP::ClientEngine::attach_to(...)";
...
void ClientEngine::attach(PacketEndpointDevice::ConnectionProc *proc)
 {
  host.attach(proc);
 }

void ClientEngine::detach()
 {
  host.detach();
 }
...
const Unid ClientDevice::TypeUnid={{0xA2EE'277B,0xE4A8'791B,0xFC0D'091F,0xABB8'8BA7,0x654B'6D8E,0x9FCB'23D0,0x77CF'8DB6,0xF2D0'E34F}};
