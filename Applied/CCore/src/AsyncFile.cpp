...
AsyncFile::AsyncFile(StrLen file_name,FileOpenFlags oflags,MSec timeout,ulen max_packets)
 : AsyncFile(timeout,max_packets)
 {
  open(file_name,oflags);
 }

AsyncFile::AsyncFile(StrLen dev_name,StrLen dev_file_name,FileOpenFlags oflags,MSec timeout,ulen max_packets)
 : AsyncFile(timeout,max_packets)
 {
...
FilePosType AsyncFile::open(StrLen dev_name,StrLen dev_file_name,FileOpenFlags oflags)
 {
  if( is_opened )
    {
     Printf(Exception,"CCore::AsyncFile::open(...) : already opened");
  ...
  file.open(packet2,dev_name,dev_file_name,oflags);
  ...
  if( !is_opened )
    {
     ...
     Printf(Exception,"CCore::AsyncFile::open(#.q;,#.q;,#;) : #;",dev_name,dev_file_name,oflags,error);
    }
...
void AsyncFile::write(FilePosType off,ulen len,Packet<uint8> packet) noexcept
 {
  if( !is_opened )
    {
     packet.complete();
...
void AsyncFileSystem::complete_time(PacketHeader *packet_)
 {
  Packet<uint8,Sys::AsyncFileSystem::CmpFileTimeExt> packet=packet_;

  auto ext=packet.getExt();

  error=ext->error;

  if( !error ) file_time=ext->file_time;

  packet.popExt().complete();
 }

Packet<uint8,Sys::AsyncFileSystem::CmpFileTimeExt> AsyncFileSystem::getPacket_time(const char *name,TimeScope time_scope)
 {
  Packet<uint8> packet=pset.get(time_scope);
  ...
  Packet<uint8,Sys::AsyncFileSystem::CmpFileTimeExt> packet2=packet.pushExt<Sys::AsyncFileSystem::CmpFileTimeExt>();

  packet2.pushCompleteFunction(function_complete_time());
...
CmpFileTimeType AsyncFileSystem::getFileUpdateTime(StrLen path)
 {
  TimeScope time_scope(timeout);

  fs.getFileUpdateTime(getPacket_time("getFileUpdateTime",time_scope),path);

  pset.wait(time_scope);

  guard("getFileUpdateTime",path);
...
CmpFileTimeType AsyncFileSystem::GetFileUpdateTime(StrLen path,MSec timeout)
 {
  SplitDevName split(path);

  if( !split )
    {
     Printf(Exception,"CCore::AsyncFileSystem::GetFileUpdateTime(#.q;,...) : bad name",path);
    }

  AsyncFileSystem dev(split.dev_name,timeout);

  return dev.getFileUpdateTime(split.dev_path);
 }
