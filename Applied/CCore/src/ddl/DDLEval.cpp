...
struct EvalContext::ConstRec : NoCopy
 {
  Value value;
  Gate *gate;
  ConstNode *node;

  ConstRec() noexcept : gate(0),node(0) {}

  void prepare(Eval &eval,ConstNode &node);
 };

struct EvalContext::LenRec : NoCopy
 {
  Value value;
  Gate *gate;
  LenNode *node;

  LenRec() noexcept : gate(0),node(0) {}
...
struct EvalContext::FieldRec : NoCopy
 {
  struct Rec : NoCopy
   {
...
void EvalContext::guardLen(ulen len)
 {
  ...
  if( CastOverflow(temp,len) )
    {
     Printf(Exception,"CCore::DDL::EvalContext::guardLen(#;) : type overflow",len);
    }
 }
