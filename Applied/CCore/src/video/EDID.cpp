...
static uint16 Build(uint8 lsb,uint16 msb) { return lsb|(msb<<8); }

static uint32 Build(uint8 lsb,uint32 b1,uint32 b2,uint32 msb) { return lsb|(b1<<8)|(b2<<16)|(msb<<24); }

static char CharInd(unsigned index) { return " ABCDEFGHIJKLMNOPQRSTUVWXYZ     "[index]; }
...
EDIDTimingDesc::EDIDTimingDesc(const uint8 buf[18])
 {
  pixel_clock=Build(buf[0],buf[1])*10u;

  hlen=Build(buf[2],buf[4]>>4);
  hblank=Build(buf[3],buf[4]&0x0Fu);

  vlen=Build(buf[5],buf[7]>>4);
  vblank=Build(buf[6],buf[7]&0x0Fu);

  hsync_off=Build(buf[8],buf[11]>>6);
  hsync_pulse=Build(buf[9],(buf[11]>>4)&0x02u);

  vsync_off=Build(buf[10]>>4,(buf[11]>>2)&0x02u);
  vsync_pulse=Build(buf[10]&0x0Fu,buf[11]&0x02u);
