...
PacketBuf & PacketBuf::operator = (PacketBuf &&obj) noexcept
 {
  if( this!=&obj )
    {
     MemFree(data);
...
PacketHeader::PacketHeader(PacketFunction complete_function) noexcept
 {
  time_stamp=0;
...
void PacketHeader::recycle(PacketFunction complete_function)
 {
  if( ext_len ) Abort("Fatal error : packet has extensions on recycle(...)");

  if( stack_len ) Abort("Fatal error : packet has complete functions on recycle(...)");
...
void PacketHeader::detach(PacketHeader *dst)
 {
  ...
  dst->pbuf=std::move(pbuf);
 }

/* struct PacketList */

void PacketList::destroy()
 {
