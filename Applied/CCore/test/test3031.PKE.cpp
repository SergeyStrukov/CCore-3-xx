   ...
   class EndpointManager : NoCopy , public Net::PSec::EndpointManager
    {
     public:
      ...
      virtual OpenErrorCode open(Net::XPoint pke_point,const Net::PSec::MasterKey &skey,Net::PSec::ClientProfilePtr &&client_profile)
       {
        Used(skey);
        Used(client_profile);
   ...
   void prepare()
    {
...

     {
      ...
      client.prepare(0,*client_id,std::move(client_key),std::move(server_key));
     }

     {
      PrimeKeyPtr server_key(createServerKey());

      server.prepare(std::move(server_key));
     }
    }
   ...
   void start()
    {
     ...
     CryptAlgoSelect temp[]={CryptAlgoSelect(CryptID_AES128,HashID_SHA256,DHGroupID_I)};

     server.start(Range(temp));

     client.start(Range(temp));
    }
...
using namespace Private_3031;

/* Testit<3031> */

template<>
const char *const Testit<3031>::Name="Test3031 PKE";

template<>
bool Testit<3031>::Main()
 {
  Engine engine;
