...
CmpFileTimeType FileSystem::getFileUpdateTime(StrLen path)
 {
  auto result=fs.getFileUpdateTime(path);

  if( result.error )
    {
     Printf(Exception,"CCore::FileSystem::getFileUpdateTime(#.q;) : #;",path,result.error);
    }
