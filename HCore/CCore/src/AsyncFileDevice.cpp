...
bool AsyncFileDevice::Engine::ReadWork::start(Async async)
 {
  ...
  if( result.pending )
    ...
    {
     ext->setError(result.error); // error or success
...
void AsyncFileDevice::Engine::ReadWork::complete(Async async)
 {
  ...
  ext->setError(file->completeRead(async,ext->buf.len)); // error or success
...
bool AsyncFileDevice::Engine::WriteWork::start(Async async)
 {
  ...
  if( result.pending )
    ...
    {
     if( result.error ) ext->setError(result.error); // on success it is initialized
...
void AsyncFileDevice::Engine::WriteWork::complete(Async async)
 {
  ...
  if( auto fe=file->completeWrite(async,packet1.getDataLen()) ) ext->setError(fe); // on success it is initialized
...
void AsyncFileDevice::FileState::close(Packet<uint8,Sys::AsyncFile::CloseExt> packet,bool preserve_file)
 {
  ...
  if( auto fe=errout.getFirst() )
    packet.getExt()->setError(fe);
  else
    packet.getExt()->noError();
...
Sys::AsyncFileState * AsyncFileDevice::createState(const ObjHook &hook) noexcept
 {
