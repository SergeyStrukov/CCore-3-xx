...
namespace Private_4029 {

/* class DataEngine */

class DataEngine : public MemBase_nocopy , public Funchor , public Net::PacketEndpointDevice::ConnectionProc
 {
  public:
  ...
  public:
   ...
   explicit DataEngine(const Net::PSec::MasterKey &key);
...
void DataEngine::connection_lost()
 {
  Printf(Con,"\n !lost connection\n");
 }

void DataEngine::connection_close()
 {
  Printf(Con,"\n !close connection\n");
 }

DataEngine::DataEngine(const Net::PSec::MasterKey &key)
...
struct ConnectParam
 {
  ...
  Net::PSec::CryptAlgoSelect getCryptAlgo() const { return Net::PSec::CryptAlgoSelect(crypt_id,hash_id,dhg_id); }

  template <class S>
  void scan(S &inp)
   {
    Scanf(inp,"#; #; #;",crypt_id,hash_id,dhg_id);
   }
 };
...
class ConnectEngine : public Funchor_nocopy , public MemBase
 {
  ...
  public:
   ...
   ConnectEngine(Net::UDPort psec_port,const ConnectParam &param);
...
ConnectEngine::ConnectEngine(Net::UDPort psec_port,const ConnectParam &param)
 : pke("pke_udp",function_pke_done())
 {
  ...
  pke.prepare(psec_port,*client_id,std::move(client_key),std::move(server_key));

  CryptAlgoSelect temp[]={param.getCryptAlgo()};

  pke.start(Range(temp));
...
class UDPEngine : NoCopy
 {
  ...
  public:
   ...
   explicit UDPEngine(Net::UDPort pke_port=Net::PKEClientUDPort,Net::UDPort psec_port=Net::PSecClientUDPort)
   ...
   Net::UDPort getPSecPort() const { return psec_udp.getDevicePort(); }
 };
...
void CommandEngine::cmd_udp(StrLen arg)
 {
  if( +engine )
    {
     engine->close();
  ...
  if( +arg )
    {
     ...
     Scanf(inp," #; #; #;",pke_port,psec_port,EndOfScan);
     ...
     try
       {
        ...
        Printf(Con,"UDP shutdown\n");

        udp.set(new UDPEngine(pke_port,psec_port));
...
void CommandEngine::cmd_open(StrLen arg)
 {
  ...
  try
    {
     engine.set(new ConnectEngine(udp->getPSecPort(),param));
    }
  catch(CatchType)
    {
     return;
...
void CommandEngine::cmd_help(StrLen)
 {
  Putobj(Con,"commands: udp open close test stat help exit\n");
 }

void CommandEngine::buildCmdList(CmdInput &input)
 {
  addCommand(input,"",&CommandEngine::cmd_nothing);
  addCommand(input,"udp",&CommandEngine::cmd_udp);
...
using namespace Private_4029;

/* Testit<4029> */

template<>
const char *const Testit<4029>::Name="Test4029 PSec";

template<>
bool Testit<4029>::Main()
 {
