...
using Algo = TreeUpLink<Node,unsigned>::RadixAlgo<&Node::link> ;
...
using namespace Private_2029;

/* Testit<2029> */

template<>
const char *const Testit<2029>::Name="Test2029 TreeUp";

template<>
bool Testit<2029>::Main()
 {
  OwnPtr<Engine> engine(new Engine);

  for(unsigned count=100; count ;count--) engine->run();
