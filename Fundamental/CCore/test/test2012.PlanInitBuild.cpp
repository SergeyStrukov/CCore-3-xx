   ...
   void run()
    {
     ...
     for(ulen count=Count; count ;count--) count_ok+=step();

     Printf(Con,"#; from #;\n",count_ok,Count);

     count_ok=0;

     for(ulen count=Count; count ;count--) count_ok+=step_cycle();
...
template<>
const char *const Testit<2012>::Name="Test2012 PlanInitBuild";
