...
void test2()
 {
  ...
  LinearSList2<int,NodePoolAllocator> a(std::move(list));

  for(int i=1; i<5 ;i++) list.insFirst(i+10000);
...
void testMove()
 {
  ...
  T a(std::move(*init_obj));

  for(int i=1; i<1000 ;i++) Ins(*init_obj,2*i);
...
using namespace Private_2042;

/* Testit<2042> */

template<>
const char *const Testit<2042>::Name="Test2042 LinearSList";

template<>
bool Testit<2042>::Main()
 {
  //test1();
  //test2();
  //test3();
