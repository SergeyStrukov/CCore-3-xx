  ...
  switch( type )
    {
     ...
     case Type_number_plus_number :
      {
       if( !number1.getNumber(bit) ) ok=false;

       if( !number2.getNumber(bitlen) ) ok=false;
      }
     break;
    }
                ...
                if( match.step(Atom_number) )
                  {
                   ...
                   if( match.step(Atom_begin) )
                     {
                      ...
                      if( !match.end() ) Abort("App::Parser trap3");
                     }
                   else
                     {
                      body=0;
                   ...
                   builder.add(pool.create<Element::Reg_Field>(*atom0,Element::Reg_Field::Type_number_plus_number,*atom2,*atom4,body));
