...
void GenerateReg(Reg reg,Out &out)
 {
  ...
  Printf(out,"  template <class Bar,class AddressType>\n");
  Printf(out,"  Type_#; & setTo(Bar &bar,AddressType ind) { bar.set_#;(ind,*this); return *this; }\n \n\n",name,name);
...
void GenerateBar(Bar bar,Out &out)
 {
  ...
  Printf(out,"  using AddressType = typename RW::AddressType ;\n\n");

  Printf(out,"  template <class ... TT>\n");
  Printf(out,"  explicit #;(TT && ... tt) : rw( std::forward<TT>(tt)... ) {}\n \n",name);

  Printf(out,"  template <class T>\n");
  Printf(out,"  struct Setter\n");
  Printf(out,"   {\n");
  Printf(out,"    RW &rw;\n");
  Printf(out,"    AddressType address;\n\n");

  Printf(out,"    Setter(RW &rw_,AddressType address_) : rw(rw_),address(address_) {}\n\n");
