...
FileSystem::FileSystem()
 {
  if( auto fe=fs.init() )
    {
     Printf(Exception,"PTPServerProc::FileSystem::FileSystem() : #;",fe);
    }
 }
...
FileSystem::~FileSystem()
 {
  if( auto fe=fs.exit() )
    {
     Printf(NoException,"PTPServerProc::FileSystem::~FileSystem() : #;",fe);
    }
 }
...
auto FileSystem::getFileUpdateTime(StrLen path_,StrLen point_name) -> FileUpdateTimeResult
 {
  BuildPath path(path_,point_name);
...
void HFSProc::function_GetFileUpdateTime(Inbound &inbound)
 {
  ...
  auto result=filesys.getFileUpdateTime(path,desc.getStr());
  ...
  log("HFSGetFileType(#;,#.q;) = #;",desc,path,result.file_time);
...
bool HFSProc::exist(FunctionIdType function_id)
 {
  return function_id>=Net::HFS::FunctionId_Open && function_id<=Net::HFS::FunctionId_GetFileUpdateTime ;
 }
