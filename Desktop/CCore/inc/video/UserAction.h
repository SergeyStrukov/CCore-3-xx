...
class UserAction
 {
  ...
  private:
   ...
   template <class T>
   void dispatch_other(T &obj,int) const
    {
     dispatch(obj, [] (UserAction) {} );
    }

   template <class T>
   auto dispatch_other(T &obj,NothingType) const -> decltype( obj.react_other(*this) )
    {
     dispatch(obj, [&obj] (UserAction action) { obj.react_other(action); } );
    }
