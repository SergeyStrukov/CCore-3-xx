...
/* class MixColorWindow */

void MixColorWindow::preparePalette()
 {
  ulen size=len*len;
...
class MixColorWindow::Art : public SmoothDrawArt
 {
  public:
...
void MixColorWindow::drawCap(Art &art,MPoint center,VColor color,bool select) const
 {
...
class MixColorWindow::WhiteField : public TwoField
 {
  public:
...
auto MixColorWindow::getItem(Point point) const -> ItemIndex
 {
...
VColor MixColorWindow::getColor(ItemIndex item) const
 {
  switch( item )
    {
...
void MixColorWindow::setColor(ItemIndex item,VColor vc)
 {
  switch( item )
    {
     case Item_value :
...
void MixColorWindow::copy(VColor vc)
 {
...
bool MixColorWindow::past(VColor &ret)
 {
  bool ok = false ;
...
MixColorWindow::MixColorWindow(SubWindowHost &host,const Config &cfg_)
 : SubWindow(host),
   cfg(cfg_),
...
MixColorWindow::~MixColorWindow()
 {
 }

 // methods

Point MixColorWindow::getMinSize() const
 {
  Coord radius=+cfg.radius;
  Coord space_dxy=+cfg.space_dxy;
  Coord len=+cfg.len;
...
void MixColorWindow::setColor(VColor value_)
 {
  value=value_;
...
bool MixColorWindow::isGoodSize(Point size) const
 {
  return size>=getMinSize();
 }

void MixColorWindow::layout()
 {
  Coord radius=+cfg.radius;
  Coord space_dxy=+cfg.space_dxy;
...
void MixColorWindow::open()
 {
  item=Item_value;
 }

void MixColorWindow::close()
 {
 }

 // keyboard

FocusType MixColorWindow::askFocus() const
 {
  return FocusOk;
 }

void MixColorWindow::gainFocus()
 {
  if( Change(focus,true) ) redraw();
 }

void MixColorWindow::looseFocus()
 {
...
void MixColorWindow::looseCapture()
 {
 }

MouseShape MixColorWindow::getMouseShape(Point,KeyMod) const
 {
  return Mouse_Arrow;
 }
...
void MixColorWindow::react(UserAction action)
 {
  action.dispatch(*this);
 }

void MixColorWindow::react_Key(VKey vkey,KeyMod kmod)
 {
  if( kmod&KeyMod_Ctrl )
    {
     switch( vkey )
...
void MixColorWindow::react_LeftClick(Point point,MouseKey mkey)
 {
...
void MixColorWindow::react_RightClick(Point point,MouseKey mkey)
 {
  if( auto item_=getItem(point) )
    {
...
void MixColorWindow::react_Move(Point point,MouseKey mkey)
 {
...
void MixColorWindow::react_Wheel(Point point,MouseKey,Coord delta)
 {
  if( mixUp_place.contains(point) )
    {
     Coord mix_len=+cfg.mix_len;
